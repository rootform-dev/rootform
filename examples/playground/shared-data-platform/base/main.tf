...
# Messaging: raw events are enriched and loaded through push subscriptions; GKE consumers pull.
...
resource "kubernetes_service_account_v1" "stream_consumer" {
  metadata {
...
resource "kubernetes_service_account_v1" "session_windower" {
  metadata {
      ...
      spec {
        ...
        container {
          ...
          env {
            ...
            value = "feature-store.batch.svc.cluster.local"
          }
...
resource "kubernetes_deployment_v1" "stream_consumer" {
  metadata {
    name      = "stream-consumer"
    namespace = kubernetes_namespace_v1.streaming.metadata[0].name

    labels = {
      "app.kubernetes.io/name"      = "stream-consumer"
  ...
  spec {
    ...
    template {
      ...
      spec {
        service_account_name = kubernetes_service_account_v1.stream_consumer.metadata[0].name

        container {
          name  = "stream-consumer"
          image = "europe-west1-docker.pkg.dev/plt-data-prod-4a7c/data-platform/stream-consumer:2026.09.1"

          env {
            name  = "PUBSUB_SUBSCRIPTION"
            value = google_pubsub_subscription.events_raw_stream_consumer.name
          }
...
resource "kubernetes_deployment_v1" "session_windower" {
  metadata {
    name      = "session-windower"
    namespace = kubernetes_namespace_v1.streaming.metadata[0].name

    labels = {
      "app.kubernetes.io/name"      = "session-windower"
      "app.kubernetes.io/component" = "consumer"
      "app.kubernetes.io/part-of"   = "data-platform"
    }
  }
  ...
  spec {
    ...
    template {
      ...
      spec {
        service_account_name = kubernetes_service_account_v1.session_windower.metadata[0].name

        container {
          name  = "session-windower"
          image = "europe-west1-docker.pkg.dev/plt-data-prod-4a7c/data-platform/session-windower:2026.09.1"

          env {
            name  = "PUBSUB_SUBSCRIPTION"
            value = google_pubsub_subscription.events_enriched_sessions.name
          }
