...
export const IMAGE_REFERENCE = "ghcr.io/rootform-dev/rootform";
export const IMAGE_SBOM_GENERATOR =
  "docker.io/docker/buildkit-syft-scanner:stable-1@sha256:ae4f3b554449e7e25548e7d8ccc029d17357348e30c6e3df01b92bc93654d6a9";
const IMAGE_WORKDIR = "/workspace";
const IMAGE_HOME = "/home/rootform/.rootform";
const IMAGE_USER_HOME = "/home/rootform";
const IMAGE_USER = "65532:65532";
const IMAGE_BINARY = "/usr/local/bin/rootform";
const IMAGE_SHARE = "/usr/local/share/rootform";
const IMAGE_COMMAND = ["rootform", "--help"];
...
export function imageBuildArguments(options: {
  ...
  const version = normalizeVersion(options.version);
  return [
    ...imageBuildBaseArguments({
      platforms: IMAGE_PLATFORMS.map((architecture) => `linux/${architecture}`).join(","),
...
function imageBuildBaseArguments(options: {
  platforms: string;
  revision: string;
  version: string;
}): string[] {
  return [
    ...
    "--build-arg",
    `ROOTFORM_VERSION=${options.version}`,
    "--label",
    `org.opencontainers.image.version=${options.version}`,
  ];
}

export function imageRuntimeBuildArguments(options: {
  architecture: ImageArchitecture;
  context: string;
  revision: string;
  tag: string;
  ...
  return [
    ...imageBuildBaseArguments({
      platforms: `linux/${options.architecture}`,
      revision: options.revision,
      version,
    }),
    "--provenance",
...
export function imagePublishArguments(options: {
  context: string;
  metadata: string;
  revision: string;
  version: string;
  ...
  return [
    ...
    "mode=max",
    "--sbom",
    `generator=${IMAGE_SBOM_GENERATOR}`,
    "--metadata-file",
    options.metadata,
    "--output",
    `type=registry,name=${IMAGE_REFERENCE}:${version},push=true,rewrite-timestamp=true`,
    options.context,
  ];
}
...
function expectedOverlay(options: {
  binarySha256: string;
  shared: Array<{ name: string; sha256: string }>;
}): Map<string, { mode: number; sha256: string }> {
  const files = new Map<string, { mode: number; sha256: string }>([
    [IMAGE_BINARY, { mode: 0o755, sha256: options.binarySha256 }],
  ]);
  for (const asset of options.shared) {
    files.set(`${IMAGE_SHARE}/${asset.name}`, { mode: 0o644, sha256: asset.sha256 });
  }
  return files;
}
...
export function verifyImageArchive(options: {
  ...
  for (const variant of image.variants) {
    ...
    if (JSON.stringify(variant.configuration.command) !== JSON.stringify(IMAGE_COMMAND)) {
      throw new Error("image default command drifted");
    }
    if (variant.configuration.workingDirectory !== IMAGE_WORKDIR) {
      throw new Error("image working directory drifted");
    }
    if (variant.configuration.user !== IMAGE_USER) {
      throw new Error("image default user drifted");
    }
    if (!variant.configuration.environment.includes(`HOME=${IMAGE_USER_HOME}`)) {
      throw new Error("image HOME drifted");
    }
    ...
    const observed = new Map(
      variant.overlayFiles
        .filter(({ size }) => size > 0)
        .map(({ mode, path, sha256: digest }) => [path, { mode, sha256: digest }]),
    );
    for (const [path, expectedFile] of overlay) {
      const observedFile = observed.get(path);
      if (observedFile?.sha256 !== expectedFile.sha256 || observedFile.mode !== expectedFile.mode) {
        throw new Error(`image payload drifted: ${path}`);
      }
    }
