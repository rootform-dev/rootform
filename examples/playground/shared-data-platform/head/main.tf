...
resource "google_cloud_run_v2_service" "warehouse" {
  name     = "warehouse-api"
  location = "europe-west1"
...
resource "google_pubsub_topic" "events_v2" {
  name = "platform-events-v2"
}

resource "google_pubsub_subscription" "warehouse" {
  name  = "warehouse-events"
  topic = google_pubsub_topic.events_v2.id
}
...
resource "grafana_data_source" "profiles" {
  name   = "profiles"
  type   = "pyroscope"
  org_id = grafana_organization.observability.id
}
